...
export default function ShowcaseCoachDashboard() {
  ...
  const theme = {
    ...
    cardBg: isDark ? 'bg-[#111315] border-white/5' : 'bg-white border-orange-100 shadow-sm',
    text: isDark ? 'text-white' : 'text-slate-800',
    textMuted: isDark ? 'text-slate-400' : 'text-slate-500',
    accent: isDark ? 'text-orange-400' : 'text-orange-600',
    badge: isDark ? 'bg-orange-500/10 text-orange-300 border-orange-500/30' : 'bg-orange-100 text-orange-700 border-orange-200',
  };
  ...
  // Calculate grad year breakdown
  const gradYearCounts = roster.reduce((acc, m) => {
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className={theme.textMuted}>
              {coach?.organization_name || 'Your Showcase Organization'}
            </p>
          </div>
          ...
          <Card className={theme.cardBg}>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${theme.textMuted}`}>Total Players</p>
              </div>
            </CardContent>
          </Card>
          <Card className={theme.cardBg}>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className={`text-2xl font-bold ${theme.text}`}>{schedule.length}</p>
                <p className={`text-xs ${theme.textMuted}`}>Events</p>
              </div>
            </CardContent>
          </Card>
          <Card className={theme.cardBg}>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${theme.textMuted}`}>College Commits</p>
              </div>
            </CardContent>
          </Card>
          <Card className={theme.cardBg}>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${theme.textMuted}`}>Age Groups</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
            ...
            <CardContent>
              {roster.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className={theme.textMuted}>No players yet</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {Object.entries(gradYearCounts).sort((a, b) => Number(a[0]) - Number(b[0])).map(([year, count]) => (
                    <div
                      key={year}
                      className={`flex items-center justify-between p-3 rounded-lg ${isDark ? 'bg-slate-800/50' : 'bg-slate-50'}`}
                    >
                      <span className={`font-medium ${theme.text}`}>Class of {year}</span>
                      <Badge variant="outline" className={theme.badge}>{count} players</Badge>
                    </div>
                  ))}
                </div>
              )}
          ...
          <Card className={theme.cardBg}>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Link href="/coach/showcase/team?tab=schedule">
                <Button variant="ghost" size="sm" className={theme.textMuted}>
                  View All <ChevronRight className="w-4 h-4 ml-1" />
                </Button>
              </Link>
